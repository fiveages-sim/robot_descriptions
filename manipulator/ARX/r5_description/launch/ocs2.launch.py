...
def generate_launch_description():
    ...
    urdfFile = launch.actions.DeclareLaunchArgument(
        name='urdfFile',
        default_value=get_package_share_directory('r5_description') + '/urdf/robot.urdf'
    )

    taskFile = launch.actions.DeclareLaunchArgument(
        name='taskFile',
        default_value=get_package_share_directory('r5_description') + '/config/ocs2/task.info'
    )

    libFolder = launch.actions.DeclareLaunchArgument(
        name='libFolder',
        default_value=get_package_share_directory(
            'ocs2_mobile_manipulator') + '/auto_generated/arx_r5'
    )
